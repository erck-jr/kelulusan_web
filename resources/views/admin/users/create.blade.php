                ...
                <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-6">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required autofocus class="w-full rounded-2xl bg-slate-950/50 border border-white/10 px-4 py-3 text-slate-200 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none transition" />
                    ...
                    <div class="glass-panel rounded-2xl border border-white/10 p-5">
                        ...
                        <div class="grid gap-3 sm:grid-cols-2">
                            <label class="flex cursor-pointer items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/50 p-4 transition hover:border-indigo-500/30">
                                <input type="radio" name="role" value="admin" class="h-4 w-4 text-indigo-500 focus:ring-indigo-500" {{ old('role') === 'admin' ? 'checked' : '' }} />
                                <span class="text-sm text-slate-200">Admin</span>
                            </label>
                            <label class="flex cursor-pointer items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/50 p-4 transition hover:border-indigo-500/30">
                                <input type="radio" name="role" value="operator" class="h-4 w-4 text-indigo-500 focus:ring-indigo-500" {{ old('role', 'operator') === 'operator' ? 'checked' : '' }} />
